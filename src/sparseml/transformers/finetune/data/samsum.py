...
@TextGenerationDataset.register(name="samsum")
class SAMSumDataset(TextGenerationDataset):
    """
    Text generation class for the SAMSum dataset
    :param data_args: configuration settings for dataset loading
    :param split: split from dataset to load, for instance `test` or `train[:5%]`
    :param tokenizer: tokenizer to use on dataset
    """

    SAMPLE_TEMPLATE = "Dialogue:\n{dialogue}\n\n### Summary:\n{summary}\n"

    def __init__(self, data_args, split, tokenizer):
        data_args = deepcopy(data_args)
        data_args.dataset = "samsum"
    ...
    def get_raw_dataset(self, cache_dir: Optional[str] = None):
        ...
        def restructure_fn(sample):
            sample["text"] = self.SAMPLE_TEMPLATE.format(
                dialogue=sample["dialogue"], summary=sample["summary"]
            )
        ...
        raw_dataset = self.map(
            ...
            desc="Restructuring SAMSum Dataset",
        )
        return raw_dataset
